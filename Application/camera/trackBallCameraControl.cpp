...
void TrackBallCameraControl::onCursorEvent(const double& dXPos, const double& dYPos)
{
	...
	else if (m_bMiddleMouseDown)
	{
		...
		m_uptrCamera->m_vec3Position += m_uptrCamera->m_vec3Up * fDeltaY;
		m_uptrCamera->m_vec3Position -= m_uptrCamera->m_vec3Right * fDeltaX;
	}
...
{
	m_uptrCamera->scale(fOffset * m_fScaleSpeed);
}
...
void TrackBallCameraControl::pitch(float fAngle)
{
	...
	m_uptrCamera->m_vec3Position = rotateMat * glm::vec4(m_uptrCamera->m_vec3Position, 1.0f);
}
